...
const roles = ['host', 'admin']

const UpdateUserRoleModal = ({ setIsOpen, isOpen, handleModal, user }) => {
          ...
          <div className='flex min-h-full items-center justify-center p-4 text-center'>
            ...
            >
              <DialogPanel className='w-full h-56 max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all'>
                ...
                >
                  Update User <span className='text-[#F73E7B]'>Role</span>
                </DialogTitle>
                      ...
                      >
                        <ListboxOptions className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
                          {roles.map((role, roleIdx) => (
                            <ListboxOption
                              key={roleIdx}
                              className='relative cursor-default select-none py-2 pl-10 pr-4 text-gray-900 data-[focus]:bg-[#F73E7B]  data-[focus]:text-white'
                              value={role}
                            >
                              {({ selected }) => (
                                <>
                ...
                <div className='flex mt-2 justify-center gap-5'>
                  <button
                    type='button'
                    className='inline-flex justify-center rounded-md border border-transparent bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2'
